...
現在の設定は **1日6回 × (本文+URLリプライ) = 12ポスト/日、約360ポスト/月**です。

X API の従量課金(PPU)契約ではクレジット残高が必要で、残高が尽きると API は
`402 credits depleted` を返します。この場合ジョブは失敗させず、その回の投稿を見送って
未投稿のままキューに残します(残高を補充すれば以降の実行から再開されます)。
投稿数を減らしたいときは `X_POST_SLOTS` のスロットを減らしてください。
